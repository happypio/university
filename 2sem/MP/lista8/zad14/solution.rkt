#lang racket
; PRACA GRUPOWA PIOTR PIESIAK, MARCIN BAADOWSKI, FILIP KOMOROWSKI, MAURYCY BORKOWSKI
...
; Wyrazenia ;
; --------- ;
(provide parse-apply parse-app parse-lam parse-list)
(provide parse eval)
...
(struct apply      (f args)   #:transparent)
...
(define (expr? e)
  (match e
    ...
    [(apply f l) (and (expr? f) (cons-expr?  l ))]
    [_ false]))
...
(define (parse q)
  (cond
    ...
    [(and (list? q) (eq? (length q) 3) (eq? (first q) 'apply)) ; <!!!
     (parse (parse-apply (second q) (eval (parse(third  q)))))]
...
(define (parse-apply f args)
  (define (apply-pom rest)
    (if (= 0 (length rest)) null
        (cons (car rest) (apply-pom (cdr rest)))))
  (cons f (apply-pom args)))
