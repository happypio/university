...
(provide eval-while parse-while env-empty env-lookup eval-env modify-lam)
; --------- ;
; Wyrazenia ;
; --------- ;
...
(define (eval-env e env)
  (match e
    ...
    [(clo id e env) (clo id e env)]))


(define (modify-lam e env)
  (match e
    [(const n) (const n)]
    [(binop op l r) (binop op (modify-lam l env) (modify-lam r env))]
    [(let-expr x e1 e2)
     (let-expr x (let ((v (eval-env e1 env)))
                   (cond
                     [(number? v) (const v)]
                     [(boolean? v) (const v)]
                     [(string? v) (const v)]
                     [(pair? v) (cons-expr (car v) (cdr v))]
                     [(null? v) (null-expr v)]
                     [(clo? v) v]
                     [(blackhole? v) v]))
                   (modify-lam e2 (env-add x (eval-env e1 env) env)))]
    [(letrec-expr x e1 e2) (letrec-expr x (modify-lam e1 env) (modify-lam e2 env))]
    [(var-expr x) (var-expr x)]
    [(if-expr eb et ef) (if-expr (modify-lam eb env) (modify-lam et env) (modify-lam ef env))]
    [(cons-expr e1 e2) (cons-expr (modify-lam e1 env) (modify-lam e2 env))]
    [(car-expr e) (car-expr (modify-lam e env))]
    [(cdr-expr e) (cdr-expr (modify-lam e env))]
    [(null-expr) null]
    [(null?-expr e) (null?-expr (modify-lam e env))]
    [(lam x e) (lam x (modify-lam e env))]
    [(app f e) (app (modify-lam f env) (modify-lam e env))]))
  ...
  (match e
    ...
    [(assign x e)
       (let [(tmp (eval-env e env))]
         (if (clo? tmp)
             (env-update x (modify-lam e env) env)
