...
possible_points = set()
my_mines = []
possible_sectors = [0] * 9
MINES = set()
possible_enemy_mines = set()
#FUNCTIONS FOR PRECISE MAPS:

def check_which_sector(y,x):
    for sector in range(len(SECTORS)):
        if SECTORS[sector][0][0] <= y <= SECTORS[sector][1][0] and SECTORS[sector][0][1] <= x <= SECTORS[sector][1][1]:
            return sector

def decreaseMAP():
    global exactMAP,POSSIBLE,possible_sectors
    if exactMAP[2] == 1:
        found = 0
        y = 0
        x = 0
        ...
        possible_sectors = [0] * 9
        possible_sectors[check_which_sector(y,x)] += 1
            ...
            if POSSIBLE[0][0] <= i <= POSSIBLE[1][0] and POSSIBLE[0][1] <= j <= POSSIBLE[1][1]:
                if x - shift <= j <= x + shift:
                    if (not (x - 1 <= j <= x + 1) or not (y - 1 <= i <= y + 1)) and (i,j) not in ISLANDS:
                        ...
                        possible_sectors[check_which_sector(i,j)] += 1
        ...
        if POSSIBLE[0][0] != min_corner[0]:
            ...
            for j in range(len(old_map[0])):
                if not (POSSIBLE[0][0],j + min_corner[1]) in ISLANDS and old_map[0][j] == 'P':
                    ...
                    possible_sectors[check_which_sector(POSSIBLE[0][0],j + min_corner[1])] += 1
                    sum_posibilities_in_map += 1
                else:
                    check_if_there_was_mine(POSSIBLE[0][0],j + min_corner[1])
                    tmp_line.append('X')
            tmp_map.append(tmp_line)
        ...
        for i in range(1,len(old_map)):
            ...
            for j in range(len(old_map[0])):
                if not (min_corner[0] + i - 1,min_corner[1] + j) in ISLANDS and old_map[i][j] == 'P':
                    ...
                    possible_sectors[check_which_sector(min_corner[0] + i - 1,min_corner[1] + j)] += 1
                    tmp_line.append('P')
                else:
                    check_if_there_was_mine(min_corner[0] + i - 1,min_corner[1] + j)
                    tmp_line.append('X')
            tmp_map.append(tmp_line)
    if direction == 'W':
        for i in range(len(old_map)):
            ...
            if POSSIBLE[0][1] != min_corner[1]:
                if not (min_corner[0] + i, POSSIBLE[0][1]) in ISLANDS and old_map[i][0] == 'P':
                    ...
                    possible_sectors[check_which_sector(min_corner[0] + i, POSSIBLE[0][1])] += 1
                else:
                    check_if_there_was_mine(min_corner[0] + i, POSSIBLE[0][1])
                    tmp_line.append('X')
            for j in range(1,len(old_map[0])):
                if not (min_corner[0] + i, min_corner[1] + j - 1) in ISLANDS and old_map[i][j] == 'P':
                    ...
                    possible_sectors[check_which_sector(min_corner[0] + i, min_corner[1] + j - 1)] += 1

                else:
                    check_if_there_was_mine(min_corner[0] + i, min_corner[1] + j - 1)
                    tmp_line.append('X')
            tmp_map.append(tmp_line)
    if direction == 'S':
        ...
        if POSSIBLE[1][0] != max_corner[0]:
            ...
            for j in range(len(old_map[0])):
                if not (POSSIBLE[1][0],j + min_corner[1]) in ISLANDS and old_map[len(old_map) - 1][j] == 'P':
                    ...
                    possible_sectors[check_which_sector(POSSIBLE[1][0],j + min_corner[1])] += 1
                    sum_posibilities_in_map += 1
                else:
                    check_if_there_was_mine(POSSIBLE[1][0],j + min_corner[1])
                    tmp_line.append('X')
            tmp_map.append(tmp_line)
    if direction == 'E':
        for i in range(len(old_map)):
            ...
            if POSSIBLE[1][1] != max_corner[1]:
                if not (min_corner[0] + i, POSSIBLE[1][1]) in ISLANDS and old_map[i][len(old_map[0]) - 1] == 'P':
                    ...
                    possible_sectors[check_which_sector(min_corner[0] + i, POSSIBLE[1][1])] += 1
                else:
                    check_if_there_was_mine(min_corner[0] + i, POSSIBLE[1][1])
                    tmp_line.append('X')
            tmp_map.append(tmp_line)
    result_map.append(sum_posibilities_in_map)
    result_map.append(tmp_map)
    ...
    for i in range (-4,5):
        ...
        if 0 <= new_x < new_width and not (tmp_map[0][0] + y + shift_v,tmp_map[0][1] + new_x) in ISLANDS:
            if tmp_map[3][y + shift_v][new_x] != 'P':
                ...
                possible_sectors[check_which_sector(tmp_map[0][0] + y + shift_v,tmp_map[0][1] + new_x)] += 1
                sum_posibilities +=1
    ...
    for i in range (-4,5):
        ...
        if 0 <= new_y < new_heigth and not (tmp_map[0][0] + new_y,tmp_map[0][1] + x + shift_h) in ISLANDS:
            if tmp_map[3][new_y][x + shift_h] != 'P':
                ...
                possible_sectors[check_which_sector(tmp_map[0][0] + new_y,tmp_map[0][1] + x + shift_h)] += 1
                sum_posibilities += 1
...
def find_best_sector():
    best_sector = 0
    result = 0
    for sector in range(len(possible_sectors)):
        act_sector = possible_sectors[sector]
        for mine in my_mines:
            if check_which_sector(mine[0],mine[1]) == sector:
                act_sector -= 2
        if act_sector > best_sector:
            best_sector = act_sector
            result = sector
    return best_sector,result + 1

def missed_sector(sector):
    global exactMAP
    global possible_sectors
    corner_min = exactMAP[0]
    corner_max = exactMAP[1]
    result_map = exactMAP[3]
    for i in range (SECTORS[sector - 1][0][0],SECTORS[sector - 1][1][0] + 1):
        for j in range (SECTORS[sector - 1][0][1],SECTORS[sector - 1][1][1] + 1):
            if corner_min[0] <= i <= corner_max[0] and corner_min[1] <= j <= corner_max[1]:
                if result_map[i - corner_min[0] ][j - corner_min[1]] == 'P':
                    if (i,j) not in possible_points:
                        print("bug not in possible_points",i,j,exactMAP)
...
def enemy_mine():
    ...
    for i in range(len(tmp_map)):
        for j in range(len(tmp_map[0])):
            if tmp_map[i][j] == 'P':
                possible_enemy_mines.add((i,j))

def delete_mines(y,x):
    if (y,x) in possible_enemy_mines:
        possible_enemy_mines.remove((y,x))

def check_if_there_was_mine(y,x):
    new_y = y
    new_x = x
    if len(possible_enemy_mines) == 0 or len(mine_order) == 0:
        return
    i = len(mine_order) - 1
    mine_to_check = mine_order[i]
    itr = len(sequence_after_mine) - 1
    for move in reversed(sequence_after_mine):
        if itr == mine_to_check:
            if i > 0:
                i -= 1
                mine_to_check = mine_order[i]
                #remove mines from possible positions
                delete_mines(new_y,new_x)
        new_y -= move[0]
        new_x -= move[1]
        itr -= 1
...
def is_there_mine(y,x):
    for i in range(y - 1, y + 2):
        for j in range(x - 1, x + 2):
            if (i,j) in possible_enemy_mines:
                return True
    return False
...
def do_move(y,x):
    ...
    for move in MOVES:
        new_x = x + move[1]
        new_y = y + move[0]
        if 0 <= new_y <= 14 and 0 <= new_x <= 14 and (new_y,new_x) not in ISLANDS and (new_y,new_x) not in visited and not is_there_mine(new_y,new_x):
...
def try_to_trigger():
    ...
    if best_trigger == False:
        return False
    if (best_trigger[0] / exactMAP[2]) >= 0.25:
        return best_trigger[1]
    return False

def input_orders():
    global opponent_orders,did_en_shoot,did_en_surface,did_en_sonar
...
def react_to_orders(life):
    global enshoot,opponent_orders,ensurface,first_mine,mine_order
    for order in opponent_orders:
        if order[0] == 'MOVE':
            global act_move,moved_first,sequence_after_mine
            act_move = convert_to_move(order[1])
            enemy_move(order[1])
            if first_mine == 1:
                sequence_after_mine.append(act_move)
        else:
            if order[0] == 'TORPEDO':
                global shoots
                ...
                if order[0] == 'SURFACE':
                    ...
                    if order[0] == 'SILENCE':
                        ...
                        enemy_silence()
                        mine_order.clear()
                        sequence_after_mine.clear()
                        firs_mine = 0
                    else:
                        if order[0] == 'MINE':
                            enemy_mine()
                            mine_order.append(len(sequence_after_mine) - 1)
                            first_mine = 1
                        else:
                            if order[0] == 'TRIGGER':
                                tmp_y = order[2]
                                tmp_x = order[1]
                                if (tmp_y,tmp_x) in possible_enemy_mines:
                                    possible_enemy_mines.remove((tmp_y,tmp_x))
...
def try_to_shoot(y,x):
    ...
    if best_shoots[0][0] / exactMAP[2] > 0.6:
        return best_shoots[0][1]
    return False

def time_to_evacuate(life,torpedo,y,x):
    global shoots_after_evacuate,time_to_silence
    if mylife_b4 - life > 0 or torpedo != 0 or shoots_after_evacuate >=3 or time_to_silence >= 20 or did_en_sonar == 1:
        shoots_after_evacuate,time_to_silence = 0,0
        return evacuate(y,x)
    return False

def try_to_sonar():
    best_sector = find_best_sector()
    if best_sector[0] < 0:
        return False
    if (best_sector[0]/exactMAP[2]) >= 0.15:
        return best_sector[1]
    return False

def check_sonar(result):
    if result == 'NA':
        return
    if result == 'Y':
        surface(MYSONAR)
    if result == 'N':
        missed_sector(MYSONAR)
...
while True:
    ...
    if mine_cooldown == 0 and (silence_cooldown == 6 or time_to_silence/3 <= 6):
    ...
    mymine = 0
    if sonar_cooldown == 0:
        do_sonar = try_to_sonar()
        if do_sonar != False:
            orders.append('SONAR' + ' ' + str(do_sonar))
            MYSONAR = do_sonar
            sonar_cooldown = 4
            time_to_sonar = 0
    if len(my_mines) > 0:
        ...
        if torpedo_cooldown > 0:
            ...
            if silence_cooldown > 0:
                ...
                if mine_cooldown > 0:
                    ...
                    if sonar_cooldown > 0:
                        orders.append('MOVE' + ' ' + my_move + ' ' + 'SONAR')
                    else:
