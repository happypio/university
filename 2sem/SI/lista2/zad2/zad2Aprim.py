...
def min_dist(block):
    ...
    for goal in goals:
        ...
        dist = min(tmp, dist)
    return dist
def heuristic(blocks_positions):
    result = 0
    for block in blocks_positions:
        result += min_dist(block)
    return result

def A_star_solution(y,x):
    queue = [] #priority queue
    heapq.heapify(queue)
    heapq.heappush(queue, (heuristic(boxes),(y,x,boxes,good_boxes,[]))) #state: position, boxes, good boxes and saved_moves
                ...
                if board[new_y][new_x] != 'W':
                    ...
                    if (new_y,new_x) in act_boxes:
                        ...
                        if can_move_box(new_y,new_x,act_boxes,move[0],move[1]):
                            ...
                            if win(new_act_good_boxes):
                                ...
                                if not (new_y,new_x,tuple(new_act_boxes),tuple(new_act_good_boxes)) in STATES:
                                    ...
                                    heapq.heappush(queue, (len(saved_moves) + 1 + heuristic(new_act_boxes),(new_y,new_x,new_act_boxes,new_act_good_boxes,saved_moves + [convert_move(move)])))
                    else:
                        ...
                        if not (new_y, new_x, tuple(act_boxes),tuple(act_good_boxes)) in STATES:
                            ...
                            heapq.heappush(queue, (len(saved_moves) + 1 + heuristic(act_boxes), (new_y, new_x, act_boxes,act_good_boxes,saved_moves + [convert_move(move)])))


input()
#print(board,start,WIDTH,HEIGTH)
A_star_solution(start[0],start[1])
output()
#print(ANS)
